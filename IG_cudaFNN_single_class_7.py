...
def test2(fname, loader_train, device):
    ...
    for kkkk, (X, y) in enumerate(loader_train):
        ...
        total = total * delta.view(1, 1, BS, 1, 28, 28)
        ...
        if kkkk % 5 == 0:
            # pickle.dump(ig_now.clone().detach().cpu(), open("ig_single_class0_new_%d.pkl" % kkkk, "wb"))
            pickle.dump(ig_now.clone().detach().cpu(), open("ig_single_class_7_new_%d.pkl" % kkkk, "wb"))
            pickle.dump(ig_now_all_dim.clone().detach().cpu(), open("ig_single_class_7_all_dim_new_%d.pkl" % kkkk, "wb"))
        # exit(0)
