...
def main():
    ...
    # f = open("class_7_in_out_black_rank_by_value_reversed.csv", "w")
    # f = open("class_7_in_out_black_rank_by_value.csv", "w")
    f = open("class_7_in_out_black_rank_by_ratio.csv", "w")
    # f = open("class_7_in_out_black_random.csv", "w")

    a, b, c, d = 13, 16, 20, 26

    ig = pickle.load(open("ig_single_class_7_all_dim_new_45.pkl", "rb"))
    ig2 = ig[0].clone()
    ...
    for i in range(64):
        ...
        if (ig[i].item() >0) and (ig2[i].item() - ig[i].item() <0):
            w.append((100000, i))
        w.append(( ig[i].item() / (eps + ig2[i].item() - ig[i].item()) , i))
    ...
    for cc in range(64):
        ...
        # idx = torch.from_numpy(np.random.choice(np.arange(64),cc,replace=False)).long()
        # if len(idx) != 0:
        #     mask[:, idx] = 0
