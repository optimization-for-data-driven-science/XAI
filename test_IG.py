...
def main(args):
    ...
    test2(fname, loader_test, device)

    print("Training done, model save to %s :)" % fname)



def test2(fname, loader_test, device):
    ...
    for i in range(20):
        for j in range(24):
            for k in range(24):
                w.append((ig[0][i][j][k].item(), i, j, k))
    ...
    mask = torch.zeros((4, 20, 24, 24)) + 1
    for i in range(int(p * 20 * 24 * 24)):
        mask[0][w[i][1]][w[i][2]][w[i][3]] = 0
        mask[1][w[i][1]][w[i][2]][w[i][3]] = 0
        mask[2][w[i][1]][w[i][2]][w[i][3]] = 0
        mask[3][w[i][1]][w[i][2]][w[i][3]] = 0
