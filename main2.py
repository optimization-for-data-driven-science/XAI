...
def main(args):
	...
	test2(fname, loader_train, dtype)

	print("Training done, model save to %s :)" % fname)



def test2(fname, loader_train, dtype):
	...
	for X_, y_ in loader_train:
		...
		total = torch.zeros((4, 20, 24, 24, 4, 1, 28, 28))
		...
		for i in tqdm(range(steps)):
			...
			firstterm = torch.autograd.grad(loss, tmp_)[0].clone().detach()
			# print(firstterm.shape)
			# print(secondterm.shape)

			prod = firstterm.view(4, 20, 24, 24, 1, 1, 1, 1) * secondterm
			total += prod
